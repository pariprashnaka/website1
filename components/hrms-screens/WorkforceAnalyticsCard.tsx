export default function WorkforceAnalyticsCard() {
  const stats = [
    { label: "Headcount", value: "230" },
    { label: "Attrition (YTD)", value: "6.2%" },
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center p-4">
      <div className="mono text-[10px] uppercase tracking-[.06em] mb-3" style={{ color: "var(--color-text-muted)" }}>
        Workforce Snapshot — Illustrative
      </div>
      <div className="grid grid-cols-3 gap-2">
        {stats.map((s) => (
          <div key={s.label} className="text-center px-1.5 py-2 rounded-md" style={{ background: "var(--color-bg-secondary)" }}>
            <div className="text-[13px] font-semibold mono" style={{ color: "var(--color-accent-cyan)" }}>{s.value}</div>
