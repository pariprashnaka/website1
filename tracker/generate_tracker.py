...
SEO_TASKS = [
    ...
    ("SEQUENCING DECISION: deep SEO work waits until after production", "Site Redesign", "Launch", "P1", "Low", "Not Started", "Owner", "", "High", "Owner's explicit call: production/design/content/QA comes first. A genuine, extensive SEO push (keyword research refinement, content audits, deeper technical work) is deliberately deferred until after the site is settled and live — doing that work against still-changing pages would be wasted effort. Quick, low-effort administrative SEO tasks (Search Console, GBP, directory listings, listed below) are NOT blocked by this and can still happen anytime, since they don't depend on the site being finished. SEO fundamentals (meta tags, canonical URLs, semantic structure) continue being built in as part of normal page work, same as always — this decision is about deferring the big dedicated push, not abandoning SEO consideration entirely."),
    ("Google Search Console — verify + submit sitemap", "Off-Page SEO", "Search Console", "P0", "Low", "Not Started", "Owner", "", "High", "No longer blocked — real domain has been live for a while now, this can be done anytime"),
    ...
    ("Merge redesign from staging to main", "Site Redesign", "Launch", "P0", "Medium", "Not Started", "Owner", "", "High", "The ENTIRE redesign (theme system, 3D hero, color system, i18n foundation, robot launcher) is currently only on the staging preview URL — none of it is live for real visitors yet. Needs a final full review before merging."),
    ("Translate core page content to Hindi", "Site Redesign", "Bilingual", "P1", "High", "In Progress", "Owner", "", "High", "Infrastructure is done (real /hi/ URLs, working toggle). Only the nav labels are actually translated so far — Homepage body, all 9 Services, all 4 Products, the Case Study, About, Contact, Privacy, and Terms still need real Hindi translation. Strongly recommend native-speaker review before launch, especially for Privacy/Terms given the legal stakes."),
    ("Translate 28 blog articles to Hindi", "Site Redesign", "Bilingual", "P2", "High", "Not Started", "Owner", "", "Medium", "Explicitly deferred as a separate, later phase — a large body of content on its own, bigger than the core pages."),
    ("Chatbot Hindi/Hinglish support", "Site Redesign", "Bilingual", "P2", "High", "Not Started", "Owner", "", "Medium", "Harder problem than site translation — current chatbot uses simple keyword matching, which doesn't reliably handle Hinglish. May need a different underlying approach, not just more spreadsheet rows. Deferred to a later stage."),
    ("Logo redesign decision", "Site Redesign", "Branding", "P2", "Low", "Not Started", "Owner", "", "Low", "AI-generated S+F monogram concepts reviewed — real concerns flagged (small-size legibility, F not reading clearly on its own, differs from what's live everywhere on the actual site). Explicitly deferred to a later stage per Owner's request."),
    ("Remove stray /style-demo route", "Site Redesign", "Cleanup", "P2", "Low", "Not Started", "Owner", "", "Low", "Old 3D tilt-card demo page, confirmed still publicly live and accessible on the real deployed site. Flagged once, never actually removed."),
    ("LinkedIn Company Page", "Off-Page SEO", "Social Presence", "P1", "Low", "Done", "Owner", "", "Medium", "Use admin@systemfriendly.com. Account creation requires human verification — Claude can help with naming, bio copy, and content, not account setup itself."),
    ("Twitter / X Company Account", "Off-Page SEO", "Social Presence", "P2", "Low", "Done", "Owner", "", "Low", "Use admin@systemfriendly.com. Same limitation as above."),
    ...
    ("GA4 activation — DONE 2026-07-21", "Monitoring", "Analytics", "P0", "Low", "Done", "Owner", "", "High", "Blocked on real GA4 property ID"),
...
CHANGELOG = [
    ("2026-07-19", "MAJOR REDESIGN ARC (staging branch, not yet merged to main): Built a full light-first theme system with an optional dark 'night reading' toggle (CSS-variable-driven, cascades sitewide), replacing the site's original all-dark theme. Caught and fixed two real theme-system bugs along the way: the nav had a hardcoded dark background bypassing the variable system entirely, and several hover states used literal 'text-white' making text invisible in light mode."),
    ("2026-07-19", "Restored the original homepage content/structure from git history after an earlier full-redesign attempt drifted too far from real site content — re-learned the lesson that redesign work should re-skin existing real content, not replace it."),
    ("2026-07-19", "Built a 2D animated network diagram (real architecture nodes: Browser/Gateway/Auth/ERP/AI/CRM/Database/Queue) for the hero, with cursor-reactive data packets. Fixed a real SVG z-index bug where packet labels rendered on top of node cards instead of behind them."),
    ("2026-07-19", "Applied a coordinated vibrant color system (rotating blue/purple/amber, all pulled from the existing brand palette, nothing invented) across Services, Process, Industries, and Tech Stack sections — real card shadows and colored accents replacing flat 1px borders."),
    ("2026-07-19", "Fixed a real bug affecting every Product and Case Study visual box: a shared CSS class had a hardcoded dark gradient background baked in from before the light theme existed, so boxes stayed dark (with now-dark-on-dark invisible text) regardless of the site theme toggle."),
    ("2026-07-19", "Replaced flat 100px section padding (identical on all 10 homepage sections) with fluid clamp()-based spacing scaled to actual content density — fixed the 'too much white space' issue without guessing at one blanket number."),
    ("2026-07-19", "Added soft, low-opacity color glows to the four genuinely flat-white sections (Services, Tech Stack, Case Studies, FAQ) to address a 'looks too empty/plain in light mode' complaint — same technique already used in the hero, not a new visual language."),
    ("2026-07-19", "Added a new 'Code Showcase' section (real, working retry-with-exponential-backoff TypeScript utility, hand-syntax-highlighted, editor-window styling) to reinforce engineering-quality positioning on the homepage."),
    ("2026-07-19", "Built a full real-time 3D WebGL hero scene (Three.js) visualizing the same architecture as the 2D diagram — genuine depth/lighting, cursor-reactive particles, and a real click-triggered ripple effect with spring physics. Includes a mandatory, working fallback: screens under 1024px width or with OS-level reduced-motion enabled get a lightweight CSS gradient instead of loading Three.js at all. Fixed two real bugs post-launch: a transparent canvas was letting the light-mode white page background bleed through (fixed with a solid clear color), and mouse/click listeners were being blocked by page content stacked on top (moved to window-level listeners)."),
    ("2026-07-19", "Began bilingual (English/Hindi) infrastructure using next-intl — real locale-prefixed URLs (/hi/...) chosen deliberately over a client-side-only toggle specifically for Hindi search-engine visibility. Every page route restructured under app/[locale]/. Disabled automatic browser-language auto-detection after it was incorrectly redirecting English visitors to /hi based on browser/OS settings. Built and wired in a real EN/Hindi toggle in the nav (visible sitewide, preserves the current page when switching)."),
    ("2026-07-19", "Replaced the chatbot's static icon with an animated SVG robot character — continuous idle bob, glowing antenna sway, cursor-tracking eyes (real mouse-position math, not decorative), a grin on hover, and a smooth morph into a close icon when opened. Built entirely from code, no image assets."),
